...
class Bank:
    ...
    def choose_and_remember(self, bs: list) -> list:
        self.bees = bs
        self.R = random.sample(range(0, 2*K), K)
        return self.R

    def calc_signature(self):
        indexes_diff = set(range(0, 2*K)) - set(self.R)
        blind_signature = [math.pow(self.bees[i], 1/3) % N for i in indexes_diff]
...
class Customer:
    ...
    def __make_y(self, a: int, d: int, identifier: int) -> bytes:
        return sha256((a ^ identifier).to_bytes(2, byteorder='big')
                + d.to_bytes(2, byteorder='big')).digest()
    ...
    def make_bees(self, r: int, f_out: int, n: int) -> int:
        self.bees = [self.__make_b(math.pow(r, 3), self.__f(self.__make_x(a, c), self.__make_y(a, d, ID)), N) for (a, c, d, r) in self.quads]
...
if __name__ == '__main__':
    ...
    indexes = bank.choose_and_remember(customer.bees)
    chosen_quads = [customer.quads[i] for i in indexes]
    #bs = 
